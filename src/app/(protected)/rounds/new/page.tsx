...
export default function NewRoundPage() {
  ...
  async function handleSave(notes: string, mood: string, conditions: string, energy: string) {
    ...
    // Enforce 5-round free plan limit
    const { data: profileCheck } = await supabase
      .from('users')
      .select('subscription_status')
      .eq('id', user.id)
      .single()
    const isPro = profileCheck?.subscription_status === 'pro' || profileCheck?.subscription_status === 'team'
    if (!isPro) {
      const { count } = await supabase
        ...
        .eq('user_id', user.id)
      if ((count ?? 0) >= 5) {
        setError('Free plan is limited to 5 rounds. Upgrade to Pro for unlimited rounds.')
        setSaving(false)
        return
      }
    }
